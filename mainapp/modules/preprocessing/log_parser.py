...
def open_logs(input_file_name, path):  # convert logs into database returns cursor, needed for requests
    skip_str = "skip_this"
    no_extra_str = "no_extra"

    def get_common_info(data):
        res = str(data["context"]["user_id"]) + ",'" + data["context"]["course_id"] + "','" + data["time"] + "'"
        return res

    def write_to_db(cursor, page, event_type, extraInfo=no_extra_str):
        res = get_common_info(data)
        res += ",'" + page + "'" + extra_info
        write_to_database = "insert into " + event_type + " values (" + res + ");"
        cursor.execute(write_to_database)

    database_name = input_file_name + ".sqlite"
    input_file = open(os.path.join(path, input_file_name))
    # try:
    ...
    for line in input_file:
        ...
        if data["event_type"] in video_events + ["mobile"]:
            ...
            if extra_info != skip_str:
                ...
                page = page.split("#")[0]
                write_to_db(cursor, page, event_type, extra_info)
            ...
            if event_type == "edx.problem.hint.demandhint_displayed":
                ...
                if data["event_source"] == "server":
                    ...
                    extra_info += "}', " + str(event["grade"]) + ", " + str(event["max_grade"]) + ", " + str(
                        event["attempts"]) + ", '" + str(event["problem_id"]) + "','" + str(
                        data["context"]["module"]["display_name"]) + "'"
...
def handle_request(cursor, select_str, from_str, where_str="", order_by_str="",
                   out_file_name="results.csv"):  # handle the request and write results into out_file_name, returns out_file_name
    #
    script_str = "SELECT " + select_str + " FROM " + from_str
    # handle where conditions
    if where_str != "":
        script_str += " WHERE " + where_str
    # handle order conditions
    if order_by_str != "":
        script_str += " ORDER BY " + order_by_str
    # script_str += '""'
    cursor.execute(script_str)
    # adding labels
    out_str = ""
    if select_str == "*":
        out_str = "user_id,course_name,execute_time,event_type,page,extra_info,event_class"
    else:
        out_str = select_str
    out_str += '\n'
    # adding raws
...
# cursor = open_logs("spbu_ACADRU_spring_2018-TL")
# handle_request(cursor, "*", "pause_video", "" ,  "user_id", "example.csv")
